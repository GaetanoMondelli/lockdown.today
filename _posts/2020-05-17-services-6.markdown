...
From the config file it runs a server and a client.
The client gets the reference of the compiled contracts `client.update_contracts(factory.get_objectlist())` and assign the address 0 to `owner` and the address 1 to `george`.
The owner deployS the `MyToken` contract and `mint` 1000 tokens. The `mint` transaction method can be run only from the contract's owner, creates new tokens and assign it to the owner of the contracts.
Then the owner tries to `transfer` 10000 tokens to george's account, however he has only 1000 and the transaction fails.
